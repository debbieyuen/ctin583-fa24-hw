...
public class BarbieBirthday : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        */      
        ...
        float dotProduct = Vector3.Dot(transform.forward, relativePosition.normalized);
    ...
    {
        float distanceToPinata = Mathf.Sqrt(Mathf.Pow(barbieToPinataDistance, 2) + Mathf.Pow(pinataHeight - barbieHeight, 2));
        swingsLeft--;
        if (swingsLeft <= 0)
        {
    ...
    private void SwingBat()
    {
        ...
        Quaternion currentRotation = pinata.rotation;
        rotationX = Quaternion.Euler(90, 0, 0); 
        Quaternion firstRotation = rotationX * currentRotation;
        
        rotationY = Quaternion.Euler(0, 90, 0);
        Quaternion secondRotation = rotationY * firstRotation;



        if (Quaternion.Equals(firstRotation,secondRotation))
        {
            Debug.Log("Current rotation and pinata rotation are equal.");
        }
        else
        {
            Debug.Log("Current rotation and pinata rotation are different.");
        }
        pinata.rotation = Quaternion.Lerp(firstRotation, secondRotation, Time.deltaTime * 1f); 
    }
